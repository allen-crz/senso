...
const RegisterForm = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const { user, error } = await signUpWithEmail(formData.email, formData.password, formData.confirmPassword);
      ...
      if (error) {
        ...
        if (error.status === 500 || error.message?.includes('500')) {
          errorMessage = 'Database configuration error. This usually means:\n\n• Database triggers are not working properly\n• Row Level Security policies need adjustment\n• Missing database tables or functions\n\nPlease check your Supabase project setup.';
        } else if (error.message?.includes('Server configuration error')) {
          errorMessage = error.message + '\n\nThis typically indicates database trigger issues in Supabase.';
